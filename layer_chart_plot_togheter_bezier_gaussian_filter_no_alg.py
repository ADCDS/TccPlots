...
_2pclogs = os.listdir("./data/none")
parsed_logs = []
for i in _2pclogs:
    if(i[0] == "_"):
        continue
    parsed_logs.append(json.load(open('./data/none/' + i)))
...
"""
drawLine("Layer 1", ["Hot"], -1)
ax.set(xlabel='time (s)', ylabel='peer number', title="Bezier: Layer 1")
ax.grid()
create_dir("output/2pc/together_gaussian/")
fig.savefig("output/2pc/experiment_" + str(i) + "/together_gaussian/" + layer.lower().replace(" ",
#                                                                                     "_") + ".png")
plt.show()
fig, ax = plt.subplots()
"""
...
for layer in layers:
    ...
    ax.grid()
    create_dir("output/none/together_gaussian/")
    if layer == "Unconnected nodes":
        plt.legend(loc="upper right")
    else:
        plt.legend(loc="upper left")
    fig.savefig("output/none/together_gaussian/" + layer.lower().replace(" ",
                                                                                "_") + ".png")
    plt.show()
    fig, ax = plt.subplots()
