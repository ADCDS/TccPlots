...
def drawClassificationChart(layers, peerClassifcation, log_index=-1):
    ...
    for layer in layers:
        ...
        # marker_style = dict(linestyle=':', marker='o', markersize=2, markerfacecoloralt='tab:red', fillstyle='top')
        ax.plot(x, y, color=layerColors[layer], label=layer)
...
for type in classesColors.keys():
    ...
    ax.grid()
    create_dir("output/2pc/layer_together_gaussian/")
    plt.legend(loc="upper left")
    fig.savefig("output/2pc/layer_together_gaussian/" + type.lower().replace(" ", "_") + ".png")
    plt.show()
    fig, ax = plt.subplots()
