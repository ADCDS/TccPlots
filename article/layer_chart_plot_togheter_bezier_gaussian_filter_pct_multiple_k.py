...
for layer in layers:
    ...
    if (layer == "Unconnected nodes"):
        ...
        ax.set(xlabel='Time (s)', ylabel='Percentage of disconnected peers per class', title=k + " - " + layer)
    else:
        #ax.set(xlabel='tempo (s)', ylabel='Número de peers', title=layer)
        ax.set(xlabel='Time (s)', ylabel='Percentage of peers per class', title=k + " - " + layer)


    ax.grid()
    create_dir("output/2pc/together_gaussian_pct_k/" + k + "/")
    if layer == "Layer 1" or layer == "Layer 4" or layer == "Layer 6" or layer == "Layer 5" or layer == "Unconnected nodes":
        plt.legend(loc="upper right")
    else:
        plt.legend(loc="upper left")
    fig.savefig("output/2pc/together_gaussian_pct_k/" + k + "/" + layer.lower().replace(" ", "_") + ".png")
    plt.show()
    fig, ax = plt.subplots()
