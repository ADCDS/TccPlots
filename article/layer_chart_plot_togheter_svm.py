...
svr_rbf = SVR(kernel='rbf', C=50, gamma=0.001, epsilon=.1)
...
def drawLine(layer, peerClassifcations, log_index=-1):
    ...
    for peerClassifcation in peerClassifcations:
        ...
        svr_rbf.fit(x.reshape(-1, 1), y_orig.reshape(-1, 1))

        y = svr_rbf.predict(x.reshape(-1, 1))
