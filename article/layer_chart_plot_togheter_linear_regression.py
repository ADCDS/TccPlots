...
def drawLine(layer, peerClassifcations, log_index=-1):
    ...
    for peerClassifcation in peerClassifcations:
        ...
        regr.fit(x.reshape(-1, 1), y_orig.reshape(-1, 1))
...
for layer in layers:
    ...
    ax.grid()
    # create_dir("output/2pc/together_linear_regression/")
    if layer == "Layer 1" or layer == "Layer 4" or layer == "Layer 6" or layer == "Layer 5" or layer == "Unconnected nodes":
        plt.legend(loc="upper right")
    else:
        plt.legend(loc="upper left")
    # fig.savefig("output/2pc/together_linear_regression/" + layer.lower().replace(" ", "_") + ".png")
    plt.show()
    fig, ax = plt.subplots()